...
async function loadS3Data(filters = {}) {
    try {
        ...
        if (result.success && result.data) {
            ...
            console.log('Loaded S3 Reports:', s3Reports);
            updateS3Metrics();
            updateS3Table();
            updateS3Charts();
            updateS3Summary();
...
function updateS3Metrics() {
    const totals = s3Reports.reduce((acc, report) => {
        ...
        acc.boxes += report.s3.boxes || 0;
        
        // Debug final accumulation
        acc.sales += report.s3.sales || 0;
        acc.expenses += report.s3.expenses || 0;
        acc.cost += report.s3.cost || 0;
        acc.freightBasZam += report.s3.freight.bas_zam || 0;
        acc.freightAirCargo += report.s3.freight.air_cargo || 0;
        acc.freightT2Market += report.s3.freight.t2_market || 0;
        acc.netIncome += report.s3.net_income || 0;

        return acc;
    }, { 
        boxes: 0, sales: 0, expenses: 0, cost: 0, 
        freightBasZam: 0, freightAirCargo: 0, freightT2Market: 0, netIncome: 0 
    });
    ...
    document.getElementById('s3FreightBazam').textContent = formatCurrency(totals.freightBasZam);
    document.getElementById('s3AirCargo').textContent = formatCurrency(totals.freightAirCargo);
    document.getElementById('s3FreightT2Market').textContent = formatCurrency(totals.freightT2Market);
...
function updateS3Table() {
    ...
    tbody.innerHTML = s3Reports.map(report => {
        ...
        const profitMargin = s3Sales > 0 ? (report.s3.net_income / report.s3.sales) * 100 : 0;

        return `
            <tr>
                <td>${formatDate(report.report_date)}</td>
                <td>${report.s3.boxes}</td>
                <td>${formatCurrency(report.s3.sales)}</td>
                <td>${formatCurrency(report.s3.expenses)}</td>
                <td>${formatCurrency(report.s3.cost)}</td>
                <td>${formatCurrency(report.s3.freight.bas_zam)}</td>
                <td>${formatCurrency(report.s3.freight.air_cargo)}</td>
                <td>${formatCurrency(report.s3.freight.t2_market)}</td>
                <td class="${report.s3.net_income >= 0 ? 'text-success' : 'text-danger'}">
                    ${formatCurrency(report.s3.net_income)}
                </td>
...
function updateS3RevenueChart() {
    ...
    const revenueData = s3Reports.map(report => parseFloat(report.s3.sales) || 0);
    const costData = s3Reports.map(report => parseFloat(report.s3.cost) || 0);
...
function updateS3FreightChart() {
    ...
    const freightTotals = s3Reports.reduce((acc, report) => {
        acc.freightBasZam += report.s3.freight.bas_zam || 0;
        acc.freightAir += report.s3.freight.air_cargo || 0;
        acc.freightT2Market += report.s3.freight.t2_market || 0;

        return acc;
    }, { freightBasZam: 0, freightAir: 0, freightT2Market: 0 });

    s3FreightChart = new Chart(ctx, {
        type: 'doughnut',
        data: {
            labels: ['Freight Cargo', 'Freight Air', 'Freight T2 Market'],
            datasets: [{
                data: [
                    freightTotals.freightBasZam,
                    freightTotals.freightAir,
                    freightTotals.freightT2Market
                ],
                backgroundColor: [
                    '#3498db', // Bright blue for cargo
                    '#e74c3c', // Bright red for air
                    '#f39c12'  // Bright orange for T2 Market
                ],
                borderColor: [
                ...
                hoverBackgroundColor: [
                    ...
                    '#f39c12'  // Lighter orange on hover
                ],
                hoverBorderWidth: 4
            }]
        },
...
function updateS3Summary() {
    ...
    const totals = s3Reports.reduce((acc, report) => {
        ...
        const profitMargin = report.s3.sales > 0 ? (report.s3.net_income / report.s3.sales) * 100 : 0;
        ...
        acc.freightCost += report.s3.freight.bas_zam + report.s3.freight.air_cargo + report.s3.freight.t2_market;
